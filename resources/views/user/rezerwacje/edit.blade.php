    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="flex m-2 p-2">
                <a href= "{{route('user.rezerwacje.index')}}" class = "px-4 py-2 bg-indigo-500 hover:bg-indigo-700 rounded-lg text-white"> Powrót </a>
            </div>
            <div class="m-2 p-2 bg-slate-100 rounded">
            <div class="space-y-8 divide-y divide-gray-200 w-1/2 ">
                <form method = "POST" action ="{{ route('user.rezerwacje.update', $rezerwacje->id)}}">
                  @csrf
                  @method('PUT')
                  <div class="sm:col-span-6">
                    <label for="imie" class="block text-sm font-medium text-gray-700"> Imię </label>
                    <div class="mt-1">
                      <input type="text" id="imie" name="imie" value = "{{$rezerwacje->imie}}" class="block w-full transition duration-150 ease-in-out appearance-none bg-white border border-gray-400 rounded-md py-2 px-3 text-base leading-normal transition duration-150 ease-in-out sm:text-sm sm:leading-5 @error('imie') border-red-400 @enderror" />
                    </div>
                    @error('imie')
                  ...
                  <div class="sm:col-span-6 pt-5">
                    ...
                    <div class="mt-1">
                      <input type="text" id="nazwisko" name="nazwisko" value = "{{$rezerwacje->nazwisko}}" class="block w-full transition duration-150 ease-in-out appearance-none bg-white border border-gray-400 rounded-md py-2 px-3 text-base leading-normal transition duration-150 ease-in-out sm:text-sm sm:leading-5 @error('nazwisko') border-red-400 @enderror" />
                    </div>
                    @error('nazwisko')
                                <div class="text-sm text-red-400">{{ $message }}</div>
                             @enderror
                  ...
                  <div class="sm:col-span-6 pt-5">
                    ...
                    <div class="mt-1">
                      <input type="time" id="godzina_rezerwacji" name="godzina_rezerwacji" value = "{{$rezerwacje->godzina_rezerwacji}}" class="block w-full transition duration-150 ease-in-out appearance-none bg-white border border-gray-400 rounded-md py-2 px-3 text-base leading-normal transition duration-150 ease-in-out sm:text-sm sm:leading-5 @error('godzina_rezerwacji') border-red-400 @enderror" />
                    </div>
